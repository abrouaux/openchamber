...
| `session-cost.ts` / `reverted-cost-ledger.ts` | Recursive active subtree cost aggregates (session + descendants via `parentID`), plus a persistent runtime/directory/session scalar for successfully replaced reverted branches; frozen aggregates cover evicted settled sessions and descendant history loads remain lazy through `SessionMessageLoader` | One runtime and directory per session tree |
